...
def test_sympy__codegen__ast__FunctionDefinition():
    from sympy.codegen.ast import Assignment, Declaration, FunctionDefinition, Variable, real
    inp_x = Declaration(Variable(x, type=real))
...
def test_sympy__stats__stochastic_process_types__TransitionMatrixOf():
    ...
    from sympy.stats.stochastic_process_types import DiscreteMarkovChain, TransitionMatrixOf
...
def test_sympy__stats__stochastic_process_types__GeneratorMatrixOf():
    from sympy.matrices.expressions.matexpr import MatrixSymbol
    from sympy.stats.stochastic_process_types import ContinuousMarkovChain, GeneratorMatrixOf
...
def test_sympy__stats__stochastic_process_types__StochasticStateSpaceOf():
    from sympy.stats.stochastic_process_types import DiscreteMarkovChain, StochasticStateSpaceOf
    DMC = DiscreteMarkovChain("Y")
...
def test_sympy__physics__quantum__hilbert__DirectSumHilbertSpace():
    from sympy.physics.quantum.hilbert import ComplexSpace, DirectSumHilbertSpace, FockSpace
    c = ComplexSpace(2)
    f = FockSpace()
...
def test_sympy__physics__quantum__hilbert__TensorProductHilbertSpace():
    from sympy.physics.quantum.hilbert import ComplexSpace, FockSpace, TensorProductHilbertSpace
    c = ComplexSpace(2)
    f = FockSpace()
...
def test_sympy__physics__control__lti__MIMOFeedback():
    from sympy.physics.control import MIMOFeedback, TransferFunction, TransferFunctionMatrix
    tf1 = TransferFunction(x**2 - y**3, y - z, x)
    tf2 = TransferFunction(y - x, z + y, x)
...
def test_sympy__tensor__tensor__TensorHead():
    from sympy.tensor.tensor import TensorHead, TensorIndexType, TensorSymmetry, get_symmetric_group_sgs
...
def test_sympy__tensor__tensor__Tensor():
    from sympy.tensor.tensor import TensorHead, TensorIndexType, TensorSymmetry, get_symmetric_group_sgs, tensor_indices
    Lorentz = TensorIndexType('Lorentz', dummy_name='L')
    a, b = tensor_indices('a,b', Lorentz)
    sym = TensorSymmetry(get_symmetric_group_sgs(1))
...
def test_sympy__diffgeom__diffgeom__Differential():
    from sympy.diffgeom import BaseScalarField, CoordSystem, Differential, Manifold, Patch
    cs = CoordSystem('name', Patch('name', Manifold('name', 3)), [a, b, c])
...
def test_sympy__diffgeom__diffgeom__TensorProduct():
    from sympy.diffgeom import BaseScalarField, CoordSystem, Differential, Manifold, Patch, TensorProduct
    cs = CoordSystem('name', Patch('name', Manifold('name', 3)), [a, b, c])
...
def test_sympy__diffgeom__diffgeom__WedgeProduct():
    from sympy.diffgeom import BaseScalarField, CoordSystem, Differential, Manifold, Patch, WedgeProduct
    cs = CoordSystem('name', Patch('name', Manifold('name', 3)), [a, b, c])
    d = Differential(BaseScalarField(cs, 0))
...
def test_sympy__diffgeom__diffgeom__CovarDerivativeOp():
    from sympy.diffgeom import BaseVectorField, CoordSystem, CovarDerivativeOp, Manifold, Patch
    cs = CoordSystem('name', Patch('name', Manifold('name', 3)), [a, b, c])
    v = BaseVectorField(cs, 0)
...
def test_sympy__tensor__array__expressions__array_expressions__ArrayElementwiseApplyFunc():
    from sympy.tensor.array.expressions.array_expressions import ArrayElementwiseApplyFunc, ArraySymbol
    A = ArraySymbol("A", (4,))
