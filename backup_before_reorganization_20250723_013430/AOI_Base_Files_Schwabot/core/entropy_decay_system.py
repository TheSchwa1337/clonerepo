...
asset_distribution[signal.asset] = asset_distribution.get(
signal.asset, 0) + 1
...
try:
if len(
self.active_signals) < self.config['max_signals']:
return
...
self.logger.info(
f"Cleaned up {remove_count} old signals due to capacity")

except Exception as e:
self.logger.error(
f"Error cleaning up old signals: {e}")
