...
def create_tcell_strategy(
self,
source_asset: str,
target_asset: str,
delta_t: int,
omega_weight: float = 0.4,
xi_weight: float = 0.3,
...
self.logger.info(
f"Created T-cell strategy: {strategy_hash[:16]}... "
...
xi_a: float,
phi_b: float,
vault_pressure: float = 0.1,
) -> TCellActivationResult:
"""
...
def update_tcell_survival(
self, strategy_hash: str, roi: float, success: bool
) -> None:
"""
...
tcell.success_rate = (
tcell.success_rate * 0.9 + (1.0 if success else 0.0) * 0.1
)

# Calculate clonal expansion factor
# 𝓒 = 𝓣_f × ROI_gain × recurrence_score
clonal_factor = (
tcell.survival_score * max(0, tcell.total_roi) * tcell.success_rate
)
...
self.logger.info(
f"T-cell clonal expansion: {strategy_hash[:16]}... "
...
self.logger.info(
f"T-cell apoptosis: {strategy_hash[:16]}... "
f"(survival_score: {tcell.survival_score:.3f})"
)

else:
# Maintain current state
tcell.survival_score = max(
0.0, min(1.0, tcell.survival_score + (0.05 if success else -0.05))
)
...
"delta_t": original_tcell.delta_t
+ np.random.randint(-2, 3),  # ±2 ticks
"omega_weight": max(
0.1,
min(0.9, original_tcell.omega_weight + np.random.normal(0, 0.1)),
),
"xi_weight": max(
0.1, min(0.9, original_tcell.xi_weight + np.random.normal(0, 0.1))
),
"phi_weight": max(
0.1, min(0.9, original_tcell.phi_weight + np.random.normal(0, 0.1))
),
"timestamp": time.time(),
}
...
self.logger.info(
f"T-cell mutation: {strategy_hash[:16]}... → {new_strategy_hash[:16]}..."
)
...
def get_front_run_candidates(
self,
source_asset: str,
omega_a: float,
xi_a: float,
vault_pressures: Dict[str, float],
...
candidates.append(
(strategy_hash, activation_result.activation_probability)
)
...
if (
tcell.state == TCellState.EXHAUSTED
and tcell.survival_score < self.config["apoptosis_threshold"]
):
...
param_string = (
f"{params['source']}_{params['target']}_{params['delta_t']}_"
...
self.logger.info(
f"Created {len(default_strategies)} default T-cell strategies"
)
...
1
for tcell in self.tcell_population.values()
if tcell.state == state
)

avg_survival_score = (
np.mean(
[tcell.survival_score for tcell in self.tcell_population.values()]
)
if total_tcells > 0
else 0.0
)
...
# Factory function
def create_tcell_survival_engine(
