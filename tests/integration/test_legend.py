...
def test_loc_invalid_tuple_exception():
    ...
    with pytest.raises(
        ValueError,
        match=('loc must be string, coordinate ' 'tuple, or an integer 0-10, not \\(0.481, \'go blue\'\\)'),
    ):
...
def test_loc_invalid_list_exception():
    ...
    with pytest.raises(
        ValueError,
        match=('loc must be string, coordinate ' 'tuple, or an integer 0-10, not \\[1.1, 2.2, 3.3\\]'),
    ):
