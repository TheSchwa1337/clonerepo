...
class MatrixMapper:
    ...
    def load_matrix(self, strategy_id: str, market_data: Dict[str, Any], strategy_performance: Dict[str, Any]) -> Matrix:
        """
    ...
    def _create_matrix(self, strategy_id: str, market_data: Dict[str, Any], strategy_performance: Dict[str, Any]) -> Matrix:
        """Create a new matrix."""
        try:
    ...
    def _update_matrix(self, matrix: Matrix, market_data: Dict[str, Any], strategy_performance: Dict[str, Any]) -> Matrix:
    ...
    def _generate_core_hash_vector(self, strategy_id: str, entropy_vector: np.ndarray, inertial_mass_tensor: np.ndarray,
    ...
    def _calculate_fitness_score(self, entropy_vector: np.ndarray, oscillation_profile: np.ndarray, inertial_mass_tensor: np.ndarray,
...
def match_hash_to_matrix(hash_value: str, matrices: Dict[str, Matrix]) -> Optional[Matrix]:
    """
    Match a hash value to the best matching matrix.
    
    Args:
        hash_value: Hash value to match
        matrices: Dictionary of available matrices
        
    Returns:
        Best matching matrix or None if no match found
    """
    try:
        if not matrices:
            return None
        ...
        for matrix_id, matrix in matrices.items():
            # Calculate similarity between hash and matrix core_hash
            if matrix.core_hash:
                # Simple string similarity for now
                similarity = _calculate_hash_similarity(hash_value, matrix.core_hash)
        ...
        logger.error(f"Error matching hash to matrix: {e}")
        return None
...
def _calculate_hash_similarity(hash1: str, hash2: str) -> float:
    ...
    try:
        ...
        # Convert hashes to character arrays
        chars1 = list(hash1)
        chars2 = list(hash2)
        
        # Calculate Jaccard similarity
        intersection = len(set(chars1) & set(chars2))
        union = len(set(chars1) | set(chars2))
