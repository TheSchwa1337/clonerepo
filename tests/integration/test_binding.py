...
class TestValueRef(BaseTest):
    ...
    def test_type_name(self):
        ...
        self.assertIsNotNone(re.match(r"struct\.glob_type(\.[\d]+)?", st.global_value_type.name))
...
class TestTypeRef(BaseTest):
    ...
    def test_global_typeref_as_ir(self):
        ...
        self.assertEqual({x.name for x in _TypeKindToIRType.keys()}, set(makers.keys()) | set(skipped))
    ...
    def _check_typeref_as_ir_for_wrappers(self, asm, target_symbol):
        ...
        self.assertTrue(new_mod.get_function(declfn.name).is_declaration, msg="declfn must not have a body")
        ...
        self.assertFalse(new_mod.get_function(declfn.name).is_declaration, msg="declfn must have a body now")
        self.assertEqual(
...
class TestTypeParsing(BaseTest):
    ...
    def test_literal_struct(self):
        ...
        with self.check_parsing() as mod:
            const = ir.Constant.literal_struct([ir.IntType(32)(1), ir.IntType(32)(2)], packed=True)
