...
[current_entropy, previous_entropy, older_entropy])
...
transformed = 1.0 / \
(1.0 + np.exp(-bitmap.astype(np.float64) * weight))
folded += transformed
...
self.bitmap_history[-1], self.bitmap_history[-2]).astype(np.uint8)
else:
return self.bitmap_history[-1] if self.bitmap_history else np.array([
])
...
self.recursion_depth = max(
0, self.recursion_depth - 1)
return self.bitmap_history[-1] if self.bitmap_history else np.array([
])
...
if len(
bitmap) != self.config['bitmap_size']:
...
# Pad with zeros
padded = np.zeros(
target_size, dtype=bitmap.dtype)
padded[:len(
bitmap)] = bitmap
return padded
else:
...
self.logger.error(
f"Error resizing bitmap: {e}")
return np.zeros(
self.config['bitmap_size'], dtype=np.uint8)
...
self.logger.error(
f"Error calculating bitmap entropy: {e}")
return 0.5
...
original_size = sum(
bitmap.nbytes for bitmap in self.bitmap_history)
...
self.logger.error(
f"Error calculating compression ratio: {e}")
return 1.0
