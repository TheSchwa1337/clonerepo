...
# -*- coding: utf-8 -*-
"""
Schwabot AI Enhanced Interface - Schwabot Integration Extension
============================================================

This module extends Schwabot AI's existing Flask/HTTP interface to integrate
...
from .schwabot_ai_bridge import Schwabot AIBridge, CommandType, AICommand
...
class Schwabot AIEnhancedInterface:
    """Enhanced interface that extends Schwabot AI's existing functionality."""
    
    def __init__(self, schwabot_port: int = 5001, enhanced_port: int = 5006):
        ...
        # Initialize bridge
        self.bridge = Schwabot AIBridge(schwabot_port=schwabot_port, bridge_port=5005)
        ...
        logger.info("🔧 Schwabot AI Enhanced Interface initialized")
    
    def _setup_enhanced_routes(self):
        """Setup enhanced Flask routes that extend Schwabot AI's functionality."""
    ...
    async def _get_market_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary_request = self.bridge.schwabot_integration.SchwabotRequest(
                prompt=f"Provide a brief market summary based on this data: {json.dumps(market_data, indent=2)}",
                max_length=200,
                temperature=0.7,
                analysis_type=self.bridge.schwabot_integration.AnalysisType.MARKET_ANALYSIS
            )
            
            summary_response = await self.bridge.schwabot_integration.analyze_trading_data(summary_request)
    ...
    def start_enhanced_interface(self):
        ...
        try:
            ...
            logger.info(f"🚀 Schwabot AI Enhanced Interface started on port {self.enhanced_port}")
    ...
    def stop_enhanced_interface(self):
        ...
        try:
            self.enhanced_running = False
            logger.info("🛑 Schwabot AI Enhanced Interface stopped")
...
def get_enhanced_interface() -> Schwabot AIEnhancedInterface:
    """Get the global enhanced interface instance."""
    global _enhanced_interface
    if _enhanced_interface is None:
        _enhanced_interface = Schwabot AIEnhancedInterface()
