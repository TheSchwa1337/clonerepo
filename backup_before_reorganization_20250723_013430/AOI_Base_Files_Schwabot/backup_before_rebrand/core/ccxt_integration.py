...
self.exchange_health_metrics[exchange_id] = mathematical_health
...
mathematical_score = 0.8  # Default score
tensor_score = 0.7  # Default tensor score
entropy_value = 0.3  # Default entropy

if self.tensor_algebra:
# Calculate tensor-based score
price_tensor = np.array([bid[0] for bid in bids[:20]])
volume_tensor = np.array([bid[1] for bid in bids[:20]])

if len(price_tensor) > 0:
tensor_score = float(
np.mean(price_tensor * volume_tensor) / (best_bid * depth)
)
tensor_score = min(max(tensor_score, 0.0), 1.0)

if self.entropy_math:
# Calculate entropy-based analysis
price_volumes = [(bid[0], bid[1]) for bid in bids[:50]]
if price_volumes:
prices = [pv[0] for pv in price_volumes]
volumes = [pv[1] for pv in price_volumes]

# Simple entropy calculation
total_volume = sum(volumes)
if total_volume > 0:
probabilities = [v / total_volume for v in volumes]
entropy_value = -sum(
p * np.log2(p) for p in probabilities if p > 0
)
entropy_value = min(max(entropy_value / 10.0, 0.0), 1.0)

# Calculate overall mathematical score
mathematical_score = tensor_score * 0.6 + (1.0 - entropy_value) * 0.4
...
"tensor_analysis": {
"price_tensor_score": tensor_score,
"volume_distribution": float(
np.std([bid[1] for bid in bids[:20]]) if bids else 0
),
},
"entropy_analysis": {
"price_entropy": entropy_value,
...
"depth_score": min(depth / 1000.0, 1.0),
"liquidity_score": min(len(bids) / 100.0, 1.0),
},
},
)
...
self.logger.error(
f"❌ Error processing order book for {exchange_id}:{symbol}: {e}"
)
return None
