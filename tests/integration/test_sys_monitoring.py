    ...
    def test_monitoring_multiple_threads(self):
        ...
        def t1_work(self, q):
            try:
                ...
                cb = self.run_with_events(self.call_foo, (self.arg,), (PY_START,), tool_id=sys.monitoring.DEBUGGER_ID)
                # Check...
                self.assertEqual(len(cb), 1)
                self.check_py_start_calls(cb)
        ...
        def t2_work(self, q):
            try:
                ...
                cb = self.run_with_events(self.call_foo, (self.arg,), (PY_RETURN,), tool_id=sys.monitoring.COVERAGE_ID)
                # Check...
                self.assertEqual(len(cb), 1)
                self.check_py_return_calls(cb)
            except Exception as e:
