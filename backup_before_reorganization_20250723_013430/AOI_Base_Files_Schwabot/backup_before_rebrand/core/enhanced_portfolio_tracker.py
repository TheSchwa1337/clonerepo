...
logger.debug(
f"Updated prices for {len(price_updates)} symbols")
...
config_symbols = self.config.get(
'tracked_symbols', [])
symbols.update(config_symbols)

# Add symbols from target allocation
symbols.update(
self.rebalancing_config.target_allocation.keys())
...
logger.info(
f"✅ Synced {exchange_name} balances")
except Exception as e:
logger.error(
f"❌ Failed to sync {exchange_name}: {e}")

logger.info(
"✅ Portfolio synced with exchanges")

except Exception as e:
logger.error(
f"Exchange sync error: {e}")
...
return {
'needs_rebalancing': False, 'reason': 'Rebalancing disabled'}
...
return {
'needs_rebalancing': False, 'reason': 'Too soon since last rebalance'}
...
logger.info(
f"🔄 Rebalancing needed: {len(analysis['rebalancing_actions'])} actions")
return analysis
else:
return {
'needs_rebalancing': False, 'reason': 'Portfolio within thresholds'}

except Exception as e:
logger.error(
f"Rebalancing check error: {e}")
return {
'needs_rebalancing': False, 'reason': f'Error: {e}'}
